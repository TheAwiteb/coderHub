...
def test_get_challenges():
    ...
    assert bool(challenge) != False
    assert bool(coderhub.get_challenges(difficulty="سهل"))
    assert bool(coderhub.get_challenges(difficulty="متوسط"))
    assert bool(coderhub.get_challenges(difficulty="صعب"))
    try_catch(coderhub.get_challenges, RANDOM_TEXT)
    return challenge['result'][0]['id']

def test_search_challenges():
    assert bool(coderhub.search_challenges(word="ترتيب"))
...
def test_get_challenge_by_id():
    ...
    assert bool(coderhub.get_challenge_by_id(challenge_id=challenge_id))
    try_catch(coderhub.get_challenge_by_id, RANDOM_TEXT)

def test_get_languages():
    assert bool(coderhub.get_languages(language=None))
    assert bool(coderhub.get_languages(language="python"))
    assert bool(coderhub.get_languages(language="PyThOn"))
    assert bool(coderhub.get_languages(language="java"))
    assert bool(coderhub.get_languages(language="swift"))
    try_catch(coderhub.get_languages, RANDOM_TEXT)

def test_get_leaderBoard():
    assert bool(coderhub.get_leaderBoard(language="Python", search_type="ALL"))
    assert bool(coderhub.get_leaderBoard(language="Python", search_type="WEEKLY"))
    assert bool(coderhub.get_leaderBoard(language="Python", search_type="DAILY"))
    try_catch(coderhub.get_leaderBoard, RANDOM_TEXT, "ALL")
    try_catch(coderhub.get_leaderBoard, "Python", RANDOM_TEXT)

def test_get_profile():
    assert bool(coderhub.get_profile(username="x7md"))
    assert bool(coderhub.get_profile(username="masha"))
    try_catch(coderhub.get_profile, "Awiteb")
    try_catch(coderhub.get_profile, RANDOM_TEXT)

def test_get_user_statistics():
    assert bool(coderhub.get_user_statistics(username="x7md"))
